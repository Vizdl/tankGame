	...
	public MyPanel() {
		...
		for (int i = 0, index = 0; i < Record.getHostileTankNumber(); i++) {
			...
			if (i < 8) {
				...
				int temp = i % 6;
				newHt = new HostileTank(i * 50, temp * 50, HostileTank.LEFT, nowTankColor, 3,
						Tank.TankColorToLiveTable(nowTankColor));
			}
			newHt.setHts(hts);
			newHt.setMt(mt);
...
class Record{
	...
	private static String rootPath = System.getProperty("java.class.path");
	private static String recordPath = rootPath + "\\Record\\record.txt";
	//记录最高分文件的以文件夹src为根目录的文件路径。
	private static String highestScoreRecordPath = rootPath + "\\Record\\highestScoreRecord.txt";
	//记录之前存档的数据的以文件夹src为根目录的文件路径。
	private static String recorderPath = rootPath + "\\Record\\recorder.txt";
